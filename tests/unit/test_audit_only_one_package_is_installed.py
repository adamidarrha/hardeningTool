...
@patch.object(Centos7Audit, "_shellexec", mock_packages_one_installed)
def test_audit_only_one_package_is_installed_pass():
    state = test.audit_only_one_package_is_installed(packages=packages)
    assert state == 0


@patch.object(Centos7Audit, "_shellexec", mock_packages_both_installed)
def test_audit_only_one_package_is_installed_fail_both_installed():
    state = test.audit_only_one_package_is_installed(packages=packages)
    assert state == 1


@patch.object(Centos7Audit, "_shellexec", mock_packages_not_installed)
