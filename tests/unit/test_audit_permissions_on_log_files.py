...
@patch.object(Centos7Audit, "_shellexec", mock_audit_permissions_on_log_files_are_configured_pass)
def test_audit_permissions_on_log_files_are_configured_pass():
    state = test.audit_permissions_on_log_files()
    assert state == 0


@patch.object(Centos7Audit, "_shellexec", mock_audit_permissions_on_log_files_are_configured_fail)
